...
/**
	...
	public void bindCloudletToVmsMinMinScheduling() {
		int cloudletNum = cloudletList.size();
		int vmNum = vmList.size();
		double[][] completionTime = new double[vmNum][cloudletNum];
		int sheduledTasks = 0;

		// Populate the completion time
		for(int i=0; i < vmNum; i++) {
			for (int j=0; j < cloudletNum; j++) {
				completionTime[i][j] = cloudletList.get(j).getCloudletLength()/vmList.get(i).getMips();
			}
		}
		
		while (sheduledTasks < cloudletNum) {
			
			// Find smallest completion time
			int idx = 0; 
			int idy = 0;
			double smallestCompletionTime = completionTime[0][0];
			for(int i=0; i < vmNum; i++) {
				for (int j=0; j < cloudletNum; j++) {
					if(smallestCompletionTime > completionTime[i][j] || smallestCompletionTime == -1) {
						idx = i;
						idy = j;
						smallestCompletionTime = completionTime[i][j];
					}
				}
			}
			
			// Assign smallest completion time to the VM
			cloudletList.get(idy).setVmId(vmList.get(idx).getId());
			sheduledTasks++;
			// Remove couldlet from list by setting its completion time to -1
			for(int i=0; i < vmNum; i++) {
				completionTime[i][idy] = -1;
			}
			
			// Update completion times
			for(int i=0; i < cloudletNum; i++) {
				if(completionTime[idx][i] != -1) {
					completionTime[idx][i] = (cloudletList.get(i).getCloudletLength()/vmList.get(idx).getMips()) + smallestCompletionTime;
				}
			}
		}
	...
	public void bindCloudletToVmsMinMaxScheduling() {
		int cloudletNum = cloudletList.size();
		int vmNum = vmList.size();
		double[] vmReadyTime = new double[vmNum];
		double[][] completionTime = new double[vmNum][cloudletNum];
		int sheduledTasks = 0;
		...
		while (sheduledTasks < cloudletNum) {
			...
			// Find the vm with the smallest Rady time
			double smallestVmReadyTime = vmReadyTime[0];
			for(int i = 0; i < vmNum; i++) {
				if(vmReadyTime[i] < smallestVmReadyTime) {
					smallestVmReadyTime = vmReadyTime[i];
					idx = i;
				}
			}
			...
			// Assign largest completion time to the VM with the smallest ready time
			cloudletList.get(idy).setVmId(vmList.get(idx).getId());
			sheduledTasks++;
			// Remove couldlet from list by setting its completion time to -1
			for(int i=0; i < vmNum; i++) {
			...
			//Update Ready Time
			vmReadyTime[idx] = smallestVmReadyTime;
	...
	public void bindCloudletToVmsSufferageScheduling() {
		int cloudletNum = cloudletList.size();
		int vmNum = vmList.size();
		double[] vmReadyTime = new double[vmNum];
		double[][] completionTime = new double[cloudletNum][vmNum];
		int sheduledTasks = 0;
		
		// Populate the completion time
				for(int i=0; i < cloudletNum; i++) {
					for (int j=0; j < vmNum; j++) {
						completionTime[i][j] = cloudletList.get(i).getCloudletLength()/vmList.get(j).getMips();
					}
				}
				...
				while (sheduledTasks < cloudletNum) {
					...
					// Calculate Sufferage
					double sufferage[] = new double[cloudletNum];
					for(int i = 0; i < cloudletNum; i++) {
						sufferage[i] = calculateSufferage(completionTime[i]);
					}
					
					//Find the largest sufferage value
					int idx = 0;
					for (int i = 0; i < cloudletNum; i++) {
						if(sufferage[idx] < sufferage[i]) {
							idx = i;
						}
					}
					
					// Find the smallest completion time for the largest sufferage
					int idy = 0;
					for (int i = 0; i < vmNum; i++) {
						if(completionTime[idx][idy] < completionTime[idx][i]) {
							idy = i;
						}
					}
					
					// Assign it to the vm
					cloudletList.get(idx).setVmId(vmList.get(idy).getId());
					sheduledTasks++;
					
					double smallestCompletionTime = completionTime[idx][idy];
					// Remove couldlet from list by setting its completion time to -1
					for(int i=0; i < vmNum; i++) {
						completionTime[i][idy] = -1;
					}
					// Update completion times
					for(int i=0; i < vmNum; i++) {
						if(completionTime[i][idy] != -1) {
							completionTime[i][idy] = (cloudletList.get(i).getCloudletLength()/vmList.get(idy).getMips()) + smallestCompletionTime;
						}
					}
				}
	...
	private double calculateSufferage(double[] arr) {
		int smallest = 0;
		...
		for (int i = 0; i < arr.length; i++) {
			...
			if(arr[secondSmallest] > arr[i] && arr[smallest] < arr[secondSmallest]) {
				secondSmallest = i;
			}
		}
		return Math.abs(arr[smallest] - arr[secondSmallest]);
	}
	////////////////////////////////////////////////////////////////////////////////////
